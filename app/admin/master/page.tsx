...
export default async function Page() {
  ...
  const { data: projects } = await u.supabase.from("projects").select("*").order("name");
  ...
  return <AppShell user={u.profile}>
    ...
    <section className="card formgrid" id="create-project"><div><p className="eyebrow">Project register</p><h2>Create project</h2><p className="muted">Add the project name, reference code, and site address.</p></div><form className="formgrid" action="/api/admin/master" method="post"><input type="hidden" name="kind" value="project" /><div className="field"><label>Name</label><input name="name" required /></div><div className="field"><label>Reference code</label><input name="code" required /></div><div className="field span2"><label>Address</label><textarea name="address" /></div><button className="btn btn-primary">Create project</button></form></section>
