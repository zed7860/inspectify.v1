...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: user }, { data: companies }] = await Promise.all([
    u.supabase.from("profiles").select("*,companies(name)").eq("id", id).is("deleted_at", null).maybeSingle(),
  ...
  return (
    <AppShell user={u.profile}>
      ...
      <div className="detail-layout">
        ...
        <AdminForm className="card formgrid" action="/api/admin/users" method="post">
          ...
          <div className="field"><label>Username</label><input name="username" defaultValue={user.username || ""} minLength={3} maxLength={30} autoCapitalize="none" /><small>Leave blank to allow email login only.</small></div><div className="field"><label>Email</label><input type="email" name="email" defaultValue={user.email} required /></div>
          ...
          <div className="field"><label>Status</label><select name="isActive" defaultValue={String(user.is_active)}><option value="true">Active</option><option value="false">Disabled</option></select></div><button className="btn btn-primary">Save user details</button>
        </AdminForm>
