...
create or replace function public.has_project(pid uuid) returns boolean language sql stable security definer set search_path=public as $$select my_role()='ADMIN' or exists(select 1 from project_users where user_id=auth.uid() and project_id=pid)$$;
...
create policy project_users_read on project_users for select using(user_id=auth.uid() or my_role()='ADMIN');
...
create policy admin_audit on audit_logs for select using(my_role()='ADMIN');create policy settings_read on app_settings for select using(auth.uid() is not null);
